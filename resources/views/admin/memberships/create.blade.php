        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h1 class="text-xl font-semibold mb-4">Form Tambah Tipe Membership Baru</h1>
                    ...
                    <form method="POST" action="{{ route('admin.memberships.store') }}">
                        ...
                        <div class="mb-4">
                            <label for="video_limit" class="block text-sm font-medium text-gray-700">Batas Video
                                (kosongkan jika unlimited)</label>
                            <input type="number" name="video_limit" id="video_limit" value="{{ old('video_limit') }}"
                                min="0"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @error('video_limit')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
